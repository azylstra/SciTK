...
public class ArrayUtil
{
	...
	* @throws Exception
	*/
	public static double[] convertList1D(List in) throws Exception
	{
	...
	* @throws Exception
	*/
	public static double[][] convertList2D(List in) throws Exception
	{
		...
		if( !(in.get(0) instanceof List) )
			throw new Exception("Input to convertList2D must be List of Lists.");
		...
		for(int i=0; i < in.size(); i++)
		{
			out[i] = convertList1D((List) in.get(i));
		}
	...
	* @throws Exception
	*/
	public static double[][][] convertList3D(List in) throws Exception
	{
		// sanity check that input is not empty:
		if( in == null || in.size() == 0 )
			return null;
		// sanity check that input is a List of Lists:
		if( !(in.get(0) instanceof List) )
			throw new Exception("Input to convertList2D must be List of Lists of Lists (i.e. 3-D).");
		...
		for(int i=0; i < in.size(); i++)
		{
			out[i] = convertList2D((List) in.get(i));
		}
	...
	*/
	public static int listDimensionality(List l)
	{
		if(l.size() == 0)
			return 0;
		if ( l.size() > 0 && l.get(0) instanceof List)
			return 1 + listDimensionality((List) l.get(0));
		return 1;
	}
}