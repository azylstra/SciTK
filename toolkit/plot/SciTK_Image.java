...
public class SciTK_Image extends SciTK_Frame implements ClipboardOwner
{
	...
	*/
	...
	{
		this( ArrayUtil.convert_int_double(image_data_in), title_in);
	}
	...
	*/
	...
	{
		this( ArrayUtil.convert_float_double(raw_data_in), title_in);
	}
	...
	*/
	 ...
	 */
	public void update_data(int[][] raw_data_in)
	{
		update_data( ArrayUtil.convert_int_double(raw_data_in) );
	}
	 ...
	 */
	public void update_data(float[][] raw_data_in)
	{
		update_data( ArrayUtil.convert_float_double(raw_data_in) );
	}
	 ...
	 * @param raw_data_in the data in an i x j double array, e.g. raw_data_in[i][j] = pixel_ij
	 */
	public void update_data(double[][] raw_data_in)
	{
	...
	public void save_data()
	{
		...
		if( fc_return == JFileChooser.APPROVE_OPTION ) // user wants to save
		{
			...
			{
				Dialog_Error emsg = new Dialog_Error(" There was an error saving the file. \n" + e.getMessage());
			}
		}
	}
	...
	*/
	...
	{
		// prompt the user:
		Dialog_Prompt zoom_prompt = new Dialog_Prompt(new Prompt_Value("","Zoom %"),"Set Zoom %");
		if(zoom_prompt.get_accepted() == false) // user cancelled
			return;
		try
		{
			scale = (float)0.01*zoom_prompt.get_value().get_value_float();
		}
		catch(Exception e)
		{
			Dialog_Error emsg = new Dialog_Error(" Unable to set zoom, try again. " + '\n' + e.getMessage());
			set_zoom();
		}
	...
	public void select_point()
	{
		...
		icon_label.addMouseMotionListener(new MouseMotionAdapter() {
			...
			public void mouseMoved(MouseEvent me) {
				if(on_image(me.getPoint()))
				{
					// set point to coordinates ON THE IMAGE
					// (some fudge factor required)
					p_image = point_transform_toimage(me.getPoint());
		...
		icon_label.addMouseMotionListener(new MouseMotionAdapter() {
			...
			public void mouseMoved(MouseEvent me) {
				if(on_image(me.getPoint()))
				{
					...
					// set point to coordinates ON THE IMAGE
					// (some fudge factor required)
					r_corner1_image = point_transform_toimage(me.getPoint());
			...
			public void mouseDragged(MouseEvent me) {
				...
				if( on_image(me.getPoint()) )
				{
					...
					r_corner2_image = point_transform_toimage(me.getPoint());
		...
		icon_label.addMouseMotionListener(new MouseMotionAdapter() {
			...
			public void mouseMoved(MouseEvent me) {
				if(on_image(me.getPoint()))
				{
					...
					// set point to coordinates ON THE IMAGE
					// (some fudge factor required)
					c_point1_image = point_transform_toimage(me.getPoint());
			...
			public void mouseDragged(MouseEvent me) {
				...
				{
					// set second corner to define the rectangle:
					c_point2_image = point_transform_toimage(me.getPoint());
	...
	*/
	private Point point_transform_toimage(Point p)
	{
	...
	* @return the point after transformation
	*/
	private Point point_transform_toframe(Point p)
	{
		// get a rectangle containing coordinates of the displayed image
		Rectangle image_bounds = icon_label.getBounds();
		// The frame's panes:
	...
	*/
	private boolean on_image(Point p)
	{
		// use point_transform_toimage to get the coordinates of the image corner
		Point p2 = point_transform_toimage(new Point(0,0)); // in image
	...
	private void remove_all_listeners()
	{
	...
	private final void initUI()
	{
		...
		// zoom:
		ImageIcon menu_edit_zoom_icon = new ImageIcon(getClass().getResource("/resources/zoom-3.png"));
		JMenuItem menu_edit_zoom = new JMenuItem("Zoom",menu_edit_zoom_icon);
		...
		// call update_ui(), which handles the image stuff.
